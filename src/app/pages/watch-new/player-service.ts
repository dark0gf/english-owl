import { IEnglishTextBlocks, IPlayerFactoryResult, IVideoData, YouTubePlayer } from './interfaces';
...
export const playerServiceFactory: (videoData: IVideoData) => IPlayerFactoryResult = (videoData: IVideoData) => {
  ...
  const getCurrentText = async (): Promise<IEnglishTextBlocks> => {
    const time = await player.getCurrentTime();
    let result: IEnglishTextBlocks = {textBlocks: [], translate: ''};
    for (const tData of videoData.textData) {
      if (tData.s <= time && time <= tData.e) {
        result = {textBlocks: tData.englishTextBlocks, translate: tData.rt};
      }
    }
